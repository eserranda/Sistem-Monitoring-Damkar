...
@section('content')
    ...
    <div class="row">
        <div class="col-md-5">
            <div class="card">
                <h5 class="card-header">Edit Data Sensor</h5>
                <div class="card-body">
                    <form action="" method="POST" id="form_edit_data_sensor" novalidate>
                        <div class="mb-3">
                            <label class="form-label" for="bs-validation-name">Kode Sensor</label>
                            <input type ="hidden" class="form-control" id="id" name="id"
                                value="{{ $dataSensor->id }}" required readonly />
                            <input type="text" class="form-control" id="kode_sensor" name="kode_sensor"
                                value="{{ $dataSensor->kode_sensor }}" placeholder="Kode Sensor" required />
                        ...
                        <div class="row">
                            <div class="mb-3 col-lg-5 mb-0">
                                ...
                                <input type="text" id="latitude" name="latitude" class="form-control"
                                    placeholder="Latitude" value="{{ $dataSensor->latitude }}" required />
                            </div>

                            <div class="mb-3 col-lg-5 mb-0">
                                <label class="form-label" for="">Longitude</label>
                                <input type="text" id="longitude" name="longitude" class="form-control"
                                    placeholder="Longitude" value="{{ $dataSensor->longitude }}" required />
                            </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="tempat_sensor" name="tempat_sensor" required>
                                <option value="{{ $dataSensor->tempat_sensor }}">{{ $dataSensor->tempat_sensor }}</option>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="alamat" name="alamat" rows="2" value="{{ $dataSensor->alamat }}" required> {{ $dataSensor->alamat }}</textarea>
                            <div class="invalid-feedback"> </div>
                        </div>
    ...
    @push('script')
        <script>
            ...
            document.getElementById('form_edit_data_sensor').addEventListener('submit', async function(event) {
                event.preventDefault();
                try {
                    const csrfToken = document.querySelector('meta[name="csrf-token"]').content;
                    const response = await fetch('{{ route('sensor.update') }}', {
                        method: 'POST',
